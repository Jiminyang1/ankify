...
describe("AI prompt generation language", () => {
  it("defaults new cards and quizzes to English", () => {
    ...
    expect(card.system).toContain("JSON object");
    ...
    expect(quiz.system).toContain("JSON object");
    expect(quiz.user).toContain(
      "Generate the quiz now in English and return only the JSON object.",
    );
  });
  ...
  it("switches both prompts to Simplified Chinese when configured", () => {
    ...
    expect(quiz.user).toContain(
      "Generate the quiz now in Simplified Chinese and return only the JSON object.",
    );
  });
});
