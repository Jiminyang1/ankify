...
function buildAiCardPromptContext(args: {
  problem: Pick<
    Problem,
...
- Return only one JSON object with the \`question\` (front) and \`answer\` (back) fields.
...
- The answer must be direct and accurate enough for the user to check their understanding.`;
  ...
  const user = [
    ...
    actionLine,
    `Output language: ${outputLanguage}. Return only the JSON object and do not explain what you did.`,
  ]
    .filter(Boolean)
    .join("\n\n");
