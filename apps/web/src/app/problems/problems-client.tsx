...
type FilterState = {
  ...
  state: "all" | "new" | "learning" | "review" | "relearning" | "due" | "not-due" | "archived";
  tag: string;
  search: string;
};
...
export default function ProblemsPage() {
  ...
  const showArchived = filters.state === "archived";
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      try {
        ...
        setNowMs(new Date(json.serverNow).getTime());
      } catch (error) {
        if (active) {
  ...
  async function loadMore() {
    ...
    try {
      ...
      setProblems((current) => {
        ...
        for (const problem of json.problems ?? []) byId.set(problem.id, problem);
        return [...byId.values()];
      });
      setDueCount(json.dueCount ?? dueCount);
      setTotalCount(json.totalCount ?? totalCount);
      setNextCursor(json.nextCursor ?? null);
      setNowMs(new Date(json.serverNow).getTime());
    } catch (error) {
      setLoadMoreError(error instanceof Error ? error.message : "Failed to load problems");
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <Surface className="overflow-hidden">
        ...
        {filtered.length === 0 && (
          <EmptyState
            title={showArchived && problems.length === 0 ? t.problems.noArchived : t.problems.noMatches}
            description={showArchived && problems.length === 0 ? t.problems.noArchivedDescription : t.problems.noMatchesDescription}
          />
        )}
