...
/** Keyboard bridge from the page-level shortcut handler into QuizPanel state.
 *  QuizPanel refreshes this every render; the handler reads it on keydown. */
type QuizKeyInterface = {
  /** Answer the current question with choice index 0-3. No-op when the
   *  question is already answered, feedback is open, or a request is running. */
  answer: (choiceIndex: number) => void;
  /** Dismiss open feedback and move on. Returns false when nothing to advance. */
  advance: () => boolean;
};
...
export default function ReviewPage() {
  ...
  const cardCount = data?.problem?.cards.length ?? 0;
  useEffect(() => {
